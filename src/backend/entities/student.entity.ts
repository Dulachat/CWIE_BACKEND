...
export enum InternStatus {
  ...
  PracticeExperience = '1', // เตรียมฝึก
  NotSelected = '2',
}
...
export class Student {
  ...
  @Column({
    type: 'enum',
    enum: [
      InternStatus.CooperativeEducation,
      InternStatus.PracticeExperience,
      InternStatus.NotSelected,
    ],
    nullable: true,
  }) // internStatus
  ...
  @Column({
    type: 'enum',
    enum: [waitingsStatus.WAIT, waitingsStatus.INTERN, waitingsStatus.FINISH],
    default: waitingsStatus.WAIT,
    nullable: true,
  }) //waiting intern
  ...
  @Column({
    type: 'enum',
    enum: [UserStatus.ON, UserStatus.OFF],
  ...
  @ManyToOne(() => Branch, (branch) => branch.studentJoin, {
    onDelete: 'CASCADE',
  })
  @JoinColumn({ name: 'branch_id', referencedColumnName: 'id' })
  branchJoin: Branch;

  @OneToMany(() => Diary, (diary) => diary.StudentDiary, {
    onDelete: 'SET NULL',
  })
  @JoinColumn()
  Diary: Diary[];

  @OneToMany(() => EventStudent, (event) => event.student, {
    onDelete: 'SET NULL',
  })
  event: EventStudent;
}
